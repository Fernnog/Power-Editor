...
const ICON_PALETTE = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M12 2.69l.34 2.29.17.95.95.17 2.29.34-1.66 1.62-.39.38.09.94.49 5.17-4.64-2.44-.85-.45-.85.45-4.64 2.44.49-5.17.09-.94-.39-.38-1.66-1.62 2.29-.34.95-.17.17-.95L12 2.69z"></path><path d="M12 2.69l.34 2.29.17.95.95.17 2.29.34-1.66 1.62-.39.38.09.94.49 5.17-4.64-2.44-.85-.45-.85.45-4.64 2.44.49-5.17.09-.94-.39-.38-1.66-1.62 2.29-.34.95-.17.17-.95L12 2.69z"></path><path d="M22 12c0 4.42-3.58 8-8 8s-8-3.58-8-8 3.58-8 8-8"></path></svg>`;
...
// --- LÓGICA DE BACKUP E MODIFICAÇÃO DE ESTADO ---
...
function modifyStateAndBackup(modificationFn) { modificationFn(); saveStateToStorage(); debouncedTriggerAutoBackup(); render(); }
...
function renderTabActions() {
    ...
    colorBtn.onclick = (event) => { event.stopPropagation(); toggleColorPalette(tabActionsContainer, activeTab); };
    ...
    renameBtn.onclick = () => {
        ...
        if (newName && newName.trim()) { modifyStateAndBackup(() => { activeTab.name = newName.trim(); }); }
    };
...
function toggleColorPalette(anchorElement, tab) {
    ...
    TAB_COLORS.forEach(color => {
        ...
        swatch.onclick = () => { modifyStateAndBackup(() => { tab.color = color; }); palette.remove(); };
...
function addNewTab() { const name = prompt("Digite o nome da nova aba:"); if (name && name.trim()) { modifyStateAndBackup(() => { const newTab = { id: `tab-${Date.now()}`, name: name.trim(), color: getNextColor() }; appState.tabs.push(newTab); appState.activeTabId = newTab.id; }); } }
...
function handleImportFile(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = function(e) { if (!confirm("Atenção: A importação substituirá todos os seus modelos e abas atuais. Deseja continuar?")) { importFileInput.value = ''; return; } try { const importedState = JSON.parse(e.target.result); if (importedState.models && importedState.tabs && importedState.activeTabId) { appState = importedState; render(); alert('Modelos importados com sucesso!'); const now = new Date(); appState.lastBackupTimestamp = now.toISOString(); updateBackupStatus(now); } else { throw new Error('Formato de arquivo inválido.'); } } catch (error) { alert('Erro ao importar o arquivo. Verifique se é um JSON válido.'); } finally { importFileInput.value = ''; } }; reader.readAsText(file); }
function applyAllReplacements() { let content = editor.innerHTML; appState.replacements.forEach(rule => { if (rule.find) { const regex = new RegExp(rule.find.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'gi'); content = content.replace(regex, rule.replace); } }); editor.innerHTML = content; alert(`${appState.replacements.length} regra(s) de substituição aplicada(s).`); }

// --- INICIALIZAÇÃO ---
window.addEventListener('DOMContentLoaded', () => { loadStateFromStorage(); render(); const dictateBtn = document.getElementById('dictate-btn'); const dictationModal = document.getElementById('dictation-modal'); const dictationCloseBtn = document.getElementById('dictation-close-btn'); if (typeof SpeechDictation !== 'undefined' && SpeechDictation.isSupported()) { SpeechDictation.init({ micIcon: document.getElementById('dictation-mic-icon'), langSelect: document.getElementById('dictation-lang-select'), statusDisplay: document.getElementById('dictation-status'), onResult: (transcript) => { insertModelContent(transcript); } }); dictateBtn.addEventListener('click', () => { dictationModal.classList.add('visible'); }); dictationCloseBtn.addEventListener('click', () => { SpeechDictation.stop(); dictationModal.classList.remove('visible'); }); } else { dictateBtn.style.display = 'none'; } editor.addEventListener('keydown', (event) => { if (event.ctrlKey) { switch (event.key.toLowerCase()) { case 'b': event.preventDefault(); execCmd('bold'); break; case 'i': event.preventDefault(); execCmd('italic'); break; case 'u': event.preventDefault(); execCmd('underline'); break; } } else if (event.key === 'Tab') { event.preventDefault(); EditorActions.indentFirstLine(); } }); if (blockquoteBtn) { blockquoteBtn.addEventListener('click', EditorActions.formatAsBlockquote); } });
...
replaceBtn.addEventListener('click', () => { ModalManager.show({ type: 'replacementManager', title: 'Gerenciador de Substituições', initialData: { replacements: appState.replacements }, onSave: (data) => { modifyStateAndBackup(() => { appState.replacements = data.replacements; }); }, onApply: applyAllReplacements }); });